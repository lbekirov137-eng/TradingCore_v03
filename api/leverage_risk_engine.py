...
class LeverageRiskEngine:
    """
    Безопасно выбирает плечо и рассчитывает размер позиции.
    ...
    ТЕКУЩЕЕ ОГРАНИЧЕНИЕ (первый PAPER-запуск): MAX_LEVERAGE = 1.
    Логика выбора 2x/3x ниже сохранена целиком и не удалена, но
    структурно недостижима, так как итоговое плечо вычисляется как
    min(maximum_allowed_leverage, MAX_LEVERAGE). Это осознанное решение
    для первого облачного PAPER-запуска: сначала собираем честную
    статистику на 1x, и только потом отдельным явным решением
    рассматриваем повышение. Изменение этой константы — отдельное
    решение уровня риск-политики, а не рутинная правка;
    оно заблокировано тестом tests/test_leverage_cap.py.
    """
