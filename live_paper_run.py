...
# ВНИМАНИЕ НА ЕДИНИЦЫ ИЗМЕРЕНИЯ: здесь значение в ПРОЦЕНТАХ, то есть
# 0.1 == 0.1% == одна тысячная капитала, а не 10%.
#
# В репозитории сосуществуют две конвенции с одинаковым именем параметра:
#   - api/risk_engine.py:      risk_amount = balance * (risk_percent / 100)
#                              -> ПРОЦЕНТЫ, предел RiskStep.MAX_RISK_PERCENT = 0.1
#   - api/leverage_risk_engine.py: risk_amount = capital * risk_percent
#                              -> ДОЛЯ, предел MAX_RISK_PERCENT = 0.001
#
# Оба предела выражают один и тот же согласованный лимит 0.1% и оба
# fail-closed отклоняют превышение. Значение ниже попадает в первую
# конвенцию (percent), поэтому 0.1 корректно. Менять его на 0.001 НЕЛЬЗЯ:
# это молча уменьшило бы риск в 100 раз и исказило paper-статистику.
RISK_PERCENT = 0.1
